      ...
      <div className="bg-[#191b25] border border-[#434656]/40 rounded-2xl p-6 sm:p-8 shadow-2xl relative overflow-hidden backdrop-blur-xl">
        ...
        {onboardingState === 'loading' && (
          <div className="space-y-6 animate-pulse">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-[#434656]/30">
              <div className="space-y-2">
                <div className="h-6 w-48 bg-[#434656]/30 rounded-md" />
                <div className="h-4 w-72 bg-[#434656]/20 rounded-md" />
              </div>
              <div className="h-16 w-56 bg-[#11131c] rounded-xl border border-[#434656]/30" />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5">
              {[1, 2, 3, 4, 5].map((i) => (
                <div key={i} className="h-16 bg-[#11131c]/50 rounded-xl border border-[#434656]/20" />
              ))}
            </div>
          </div>
        )}
        ...
        {onboardingState === 'empty' && (
          <div className="text-center py-10 space-y-4 max-w-lg mx-auto">
            <div className="w-14 h-14 rounded-2xl bg-[#0052ff]/10 border border-[#0052ff]/30 flex items-center justify-center mx-auto text-[#4cd7f6]">
              <Sparkles className="w-7 h-7" />
            </div>
            <h3 className="text-lg font-bold font-geist text-white">Profile Onboarding Pending</h3>
            ...
            <div className="flex items-center justify-center gap-3 pt-2">
              ...
              >
                <UserCheck className="w-4 h-4 text-[#4cd7f6]" /> Complete Profile
              </button>
            </div>
          </div>
        )}
        ...
        {onboardingState === 'populated' && (
          <>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-[#434656]/30 relative z-10">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <Sparkles className="w-5 h-5 text-[#4cd7f6]" />
                  <h3 className="text-xl font-bold font-geist text-white">Complete your profile</h3>
                </div>
                <p className="text-xs sm:text-sm font-mono text-[#a1a3b8]">
                  Follow these key steps to build your AI career telemetry & matching algorithms.
                </p>
              </div>

              {/* Progress Box */}
              <div className="bg-[#11131c] border border-[#434656]/30 rounded-xl p-4 min-w-[220px] flex flex-col justify-between shadow-inner">
                <div className="flex items-center justify-between gap-3 mb-2">
                  <span className="text-xs font-mono text-[#a1a3b8] uppercase tracking-wider font-bold">Progress</span>
                  <span className="text-xs font-mono font-bold text-[#4cd7f6] bg-[#0052ff]/20 border border-[#0052ff]/40 px-3 py-1 rounded-full">
                    {completedStepsCount}/5
                  </span>
                </div>
                <div className="w-full bg-[#191b25] h-2.5 rounded-full overflow-hidden border border-[#434656]/20">
                  <div 
                    className="bg-gradient-to-r from-[#0052ff] to-[#4cd7f6] h-full transition-all duration-500 rounded-full"
            ...
            {/* Checklist Grid */}
            <div className="pt-6 grid grid-cols-1 md:grid-cols-2 gap-3.5 relative z-10">
              {onboardingSteps.map((step) => (
                <div 
                  key={step.id} 
                  onClick={() => handleStepAction(step.id)}
                  className={`flex items-start justify-between gap-3.5 p-4 rounded-xl border transition-all cursor-pointer group ${
                    step.done 
                      ? 'bg-[#11131c]/90 border-[#0052ff]/40 text-white' 
                      : 'bg-[#11131c]/50 border-[#434656]/30 hover:border-[#434656] text-[#a1a3b8] hover:text-white'
                  }`}
                >
                  <div className="flex items-start gap-3.5 flex-1 min-w-0">
                    <button
                      onClick={(e) => toggleManualStep(step.id, e)}
                      className="mt-0.5 text-[#4cd7f6] hover:scale-110 transition-transform cursor-pointer shrink-0"
                      title="Toggle status"
                    >
                      {step.done ? (
                        <CheckSquare className="w-5 h-5 text-[#0052ff] fill-[#0052ff]/20" />
                      ) : (
                        <Square className="w-5 h-5 text-[#a1a3b8] group-hover:text-white" />
                      )}
                    </button>
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        <span className={`text-sm font-bold font-geist ${step.done ? 'line-through text-white/50' : 'text-white'}`}>
                          {step.label}
                        </span>
                      </div>
                      <p className="text-xs font-mono text-[#a1a3b8] mt-1 truncate">
                        {step.subtitle}
                      </p>
                    </div>
                  </div>

                  <span className={`text-[11px] font-mono font-bold px-2.5 py-1 rounded-lg shrink-0 flex items-center gap-1 ${
                    step.done 
                      ? 'bg-green-500/10 text-green-400 border border-green-500/30' 
                      : 'bg-[#0052ff]/10 text-[#4cd7f6] border border-[#0052ff]/30 group-hover:bg-[#0052ff] group-hover:text-white transition-all'
                  }`}>
                    {step.done ? (
                      <>
            ...
            {/* Celebration Banner when 5/5 complete */}
            {completedStepsCount === 5 && (
              <div className="mt-6 p-4 rounded-xl bg-green-500/10 border border-green-500/30 text-green-300 flex items-center justify-between gap-4 font-mono text-xs">
                <div className="flex items-center gap-2">
                  <CheckCircle2 className="w-5 h-5 text-green-400 shrink-0" />
                  <span>All 5 onboarding tasks completed! Your AI career matching algorithm is running at 100% capacity.</span>
                </div>
                <button
                  onClick={() => onNavigateTab('job-suite')}
                  className="bg-green-500 hover:bg-green-400 text-black font-bold px-3.5 py-1.5 rounded-lg shrink-0 cursor-pointer"
                >
                  Explore Job Hub →
                </button>
              </div>
